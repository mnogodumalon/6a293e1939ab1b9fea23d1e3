...
interface TerminverwaltungDialogProps {
  ...
  defaultValues?: Terminverwaltung['fields'];
...
export function TerminverwaltungDialog({ open, onClose, onSubmit, defaultValues, recordId, kundenverwaltungList, enablePhotoScan = true, enablePhotoLocation = true }: TerminverwaltungDialogProps) {
  ...
  useEffect(() => {
    if (open) {
      setFields(applyDefaults((defaultValues ?? {}) as Record<string, unknown>, formEnhancements.defaults) as Partial<Terminverwaltung['fields']>);
      setPreview(null);
  ...
  async function handleAiExtract(file?: File) {
    ...
    try {
      ...
      const schema = `{\n  "kunde": string | null, // Display name from Kundenverwaltung (see <available-records>)\n  "termindatum": string | null, // YYYY-MM-DDTHH:MM\n  "terminart": LookupValue | null, // Terminart (select one key: "erstgespraech" | "beratung" | "folgetermin" | "praesentation" | "vertragsabschluss" | "sonstiges") mapping: erstgespraech=Erstgespräch, beratung=Beratung, folgetermin=Folgetermin, praesentation=Präsentation, vertragsabschluss=Vertragsabschluss, sonstiges=Sonstiges\n  "dauer": number | null, // Dauer (in Minuten)\n  "ort": string | null, // Ort / Raum\n  "status": LookupValue | null, // Status (select one key: "geplant" | "bestaetigt" | "abgesagt" | "abgeschlossen") mapping: geplant=Geplant, bestaetigt=Bestätigt, abgesagt=Abgesagt, abgeschlossen=Abgeschlossen\n  "notizen_termin": string | null, // Notizen zum Termin\n}`;
  ...
  const fieldBlocks: Record<string, React.ReactNode> = {
    ...
    'termindatum': (
      <div key="termindatum" className="space-y-1.5">
    ...
    'terminart': (
      <div key="terminart" className="space-y-1.5">
        ...
        >
          <SelectTrigger id="terminart"><SelectValue placeholder="z. B. Erstgespräch, Beratung" /></SelectTrigger>
          <SelectContent>
    ...
    'dauer': (
      <div key="dauer" className="space-y-1.5">
        ...
        <Input
          ...
          type="number"
          step="any"
          {...numberInputProps(formEnhancements, 'dauer')}
          placeholder="z. B. 60"
          value={fields.dauer !== undefined ? fields.dauer : (computedValues['dauer'] ?? '')}
          onChange={e => setFields(f => ({ ...f, dauer: clampNumberValue(formEnhancements, 'dauer', e.target.value) }))}
        />
      </div>
    ),
    'ort': (
    ...
    'notizen_termin': (
      <div key="notizen_termin" className="space-y-1.5">
        ...
        <Textarea
          id="notizen_termin"
          placeholder="Agenda, Vorbereitung, Follow-up..."
  ...
  const FIELD_LABELS: Record<string, string> = {"kunde": "Kunde", "termindatum": "Datum und Uhrzeit", "terminart": "Terminart", "dauer": "Dauer (in Minuten)", "ort": "Ort / Raum", "status": "Status", "notizen_termin": "Notizen zum Termin"};
  const CURRENCY_KEYS = new Set<string>([]);
  // Applookup-Referenz-Labels: pro applookup-Feld in dieser Form (ownKey)
  // eine Map { lookupKey: label } für ALLE Felder des Target-Schemas. Wird
  // beim Render-Walk gefiltert auf die in der computed-Formel tatsächlich
  // referenzierten lookupKeys (siehe applookupRefs unten).
  const APPLOOKUP_LABELS: Record<string, Record<string, string>> = {"kunde": {"vorname": "Vorname", "nachname": "Nachname", "email": "E-Mail-Adresse", "telefon": "Telefonnummer", "strasse": "Straße", "hausnummer": "Hausnummer", "plz": "Postleitzahl", "ort": "Ort", "notizen_kunde": "Notizen zum Kunden"}};
